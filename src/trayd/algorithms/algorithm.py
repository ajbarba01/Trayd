...
class Algorithm:
    ...
    def initialize(self, historical: HistoricalData, daily: HistoricalData, portfolio: Portfolio):
    ...
    def tick(self):
        # THIS IS BAD
        if self.historical.current_ts.normalize() != self.daily.current_ts.normalize():
            return
    ...
    def buy_up_to(self, symbol: str, shares: float = None, mod: float = 1.0, size: int = None):
        if not size: size = self.portfolio_max_size
        if not shares: shares = self.closest_share_amount(symbol, mod=mod)
        if shares:
    ...
    def short_up_to(self, symbol: str, shares: float = None, mod: float = 1.0, size: float = None):
        if not size:
    ...
    def closest_share_amount(self, symbol: str, max_price: float=None, max_size: float=None, mod: float = 1.0):
        if not max_price:
            max_price = self.company_per(max_size)
        
        max_price *= mod
        price = self.historical.get_close(symbol)
        if np.isnan(price): return 0
    ...
    def set_stop_take(self, symbol: str, price: float, take_k: float, stop_k: float):
        self.set_take_profit_ATR(symbol, price, take_k)
        self.set_stop_loss_ATR(symbol, price, stop_k)


    def set_static_stop_take(self, symbol: str, price: float, take_percent: float = None, stop_percent: float = None):
        if take_percent:
            self.portfolio.set_take_profit(symbol, price * 1 + take_percent / 100)

        if stop_percent:
            self.portfolio.set_take_profit(symbol, price * 1 - stop_percent / 100)


    def set_trailing_stop(self, symbol: str, atr: Indicator = None, k_val: float = 1):
        if not atr: atr = self.default_atr
    ...
    def report(self):
        Report.set_performance(self.name, SMA(EMA(self.portfolio.performances, 30), 60))
        self.on_report()
