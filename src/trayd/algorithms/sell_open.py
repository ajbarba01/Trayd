...
class SellOpen(Algorithm):
    ...
    def on_position_closed(self, position):
        ...
        self.daily_profits[
            position.exit_time.day_of_week
        ] += position.shares * (
            position.avg_fill_price - position.avg_entry_price
        )
        self.last_trade[position.symbol] = (
            float(position.avg_entry_price),
            str(position.entry_time),
            float(position.avg_fill_price),
            str(position.exit_time),
            float(position.avg_fill_price) / float(position.avg_entry_price)
            - 1,
        )
    ...
    def buy_all_intra(self):
        # return False
        if self.get_close("SPY") > self.sma26("SPY"):
            return False
    ...
    def buy_all(self):
        ...
        if self.get_close("SPY") < self.sma50("SPY"):
            return
        if self.get_close("OEF") < self.sma50("OEF"):
            return
    ...
    def allow_trade(self, symbols: list[str], target: str, max_corr=0.9):
        ...
        max_seen = max(self.rbc.get_corr(s, target) for s in symbols)
