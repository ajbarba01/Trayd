...
from trayd.util.helpers import format_percent, format_USD, percent_improvement, mean, max_drawdown, max_single_day_loss,annualized_risk_adjusted_return, calculate_sharpe, format_multiplier, format_float, print_numbered_list, print_dashed_list, get_correlation, get_beta
...
class Report:
    ...
    @staticmethod
    def print_report(initial: float, final: float, start_ts: pd.Timestamp, end_ts: pd.Timestamp, avg_exposure: float, start_load: float, end_load: float, total_bars: int):        
        # Format end date
        ...
        print()
        print(f"Indices Used: {",".join([idx.index_name for idx in Report.algo.indices])}")
        # print(f"Max Exposure: {format_multiplier(Report.portfolio.max_exposure)}")
        print(f"Effective Leverage: {format_multiplier(Report.portfolio.max_exposure)} of {format_multiplier(Report.portfolio.leverage)} (Deprecated)")
        print(f"Initial Value: {format_USD(initial)}")
        print(f"Final Value: {format_USD(final)} ({percent_improvement(initial, final)})")
        print(f"Annualized Return: {format_percent(annualized_return)}")
        print(f"Risk-adjusted Return: {format_percent(annualized_risk_adjusted_return(portfolio_values))}")
        ...
        print(f"Max Single Day Loss: {format_percent(max_single_day_loss(portfolio_values))}")
        print(f"Sharpe Ratio: {format_float(calculate_sharpe(portfolio_values), 1)}")
        print(f"Trades: {Report.portfolio.num_trades}")
        print(f"Slippage Percentage: {format_percent(Report.portfolio.slippage_percent / 100, 3)}")
        print(f"Total Slippage: {format_USD(-Report.portfolio.total_slippage)}")

        if Report.portfolio.total_margin_interest != 0:
            print(f"Total Margin Interest: {format_USD(-Report.portfolio.total_margin_interest)}")

        Report.print_correlations()
        Report.print_profits()
        Report.print_positions()
        print()
        print(f"Load: {(end_load - start_load):.2f} s, Runtime: {(time.time() - end_load):.2f} s, {total_bars} bars".center(80))
        print()
        print("".center(80, "-"))
    ...
    def print_correlations():
        if not Report.base or len(Report.correlations) == 0: return

        print(f"\nCORRELATIONS ({Report.base})")
        base_curve = Report.equity_curves[Report.base]
        corr_list = [f"{reference}: {format_float(get_correlation(Report.references[reference], base_curve), 0.3, flip=True)} (corr), {format_float(get_beta(Report.references[reference], base_curve), 0.1, flip=True)} (beta)" for reference in Report.correlations]
    ...
    def print_profits():
        ...
        sorted_symbols = dict(sorted(Report.portfolio.symbol_profits.items(), key=lambda item: item[1]))
        symbol_list = [f"{symbol}: {format_USD(profit)}" for symbol, profit in sorted_symbols.items()]
        print_dashed_list(symbol_list)


    @staticmethod
    def print_positions():
        if len(Report.portfolio.positions) == 0: return
        print(f"\nUNREALIZED POSITIONS ({len(Report.portfolio.positions)}):")
        symbol_list = [f"{symbol}: {position.entry_time.date()} @ {format_USD(position.last_known_price * position.shares)}" for symbol, position in Report.portfolio.positions.items()]
    ...
    def show_plots(initial: float):
        if not Report.check_valid(): return
