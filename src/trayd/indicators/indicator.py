...
class Indicator:
    ...
    def rank(
        ...
        sorted_symbols = sorted(
            values.keys(), key=lambda x: values[x], reverse=descending
        )
        if max_len:
    ...
    def compare(
        self, symbols: list[str], target_value: float, offset: int = 0
    ) -> list[str]:
        return [
            symbol
            for symbol in symbols
            if self.get(symbol, offset=offset) == target_value
        ]
    ...
    def filter(
        ...
        symbols: list[str],
        offset: int = 0,
        lower_val: float = -float("inf"),
        upper_val: float = float("inf"),
    ) -> list[str]:
        return [
            symbol
            for symbol in symbols
            if lower_val <= self.get(symbol, offset) <= upper_val
        ]
